...
### 🔥 Text-to-motion generation and retargeting demo:
![Text generation demo](https://github.com/jack111331/Keemap-Blender-Rig-ReTargeting-Addon/blob/main/assets/generate_motion.gif)
### 🔥 Trajectory control on motion generated from text prompt and retargeting demo:
![Text generation demo](https://github.com/jack111331/Keemap-Blender-Rig-ReTargeting-Addon/blob/main/assets/trajectory_control.gif)
## Make sure you are in Pose Mode to see the UI!!!!!!
Blender Rig Retargeting Addon allows mapping BVH motions of one rig to another. Works with ANY rig and allows user to map bones from one rig to another and save mapping files out to hard drive.  This script is tested and working on blender 3.3.1, if a newer version of blender breaks the script I'm sorry.

This script enables transferring from an imported rig to a target rig bones any combination of position, rotation and scale, with offsets (note scale is used for rigify finger bones that are scaled to bend the fingers).  It gives you (as much as I can) COMPLETE control over the transfer as well as allowing you to customize the transfer.  This script allows you to position IK Target bones from FK rigs,  this leaves the destination rig in animation friendly and tweakable state.  You can position the rig with one click at the desired keyframe with the 'Test All' button and 'keyframe' checked.  You can then scroll forward and do it again making keyframes as you go at exactly the right times.  This enables you to transfer and only have keyframes exactly where you want them on the target bones used by animators only.  It is meant to automate the hand process of matching a target rig to a source rig even if they are rigged entirely differently.  This tool has/is being used in a film pipeline and is meant to allow animators to do exactly what they need and no more with the control they need.  We have used this on several films and are evolving as the films get better.  We have addressed some community issues expressed on github and appreciate the feedback.

Installation Procedure:
Download the latest version zip file from the releases section of this github repo  Go to Blender-->edit(pull down from menus at top)--> Preferences, then click on the add ons button on the left of the ui.  Click on the Install button.  Select the zip file you have downloaded.  When the script shows up on the list check the box to enable it.  Make certain you DO NOT download this entire source code tree as a zip and try and install that.
...
2. After installing you are not in pose mode so you don't see the add on tab on the right side of the 3d viewport in blender.
