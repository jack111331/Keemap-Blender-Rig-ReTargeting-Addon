...
class KeeMapOptmTrajSetting(bpy.types.PropertyGroup): 
...
class KEEMAP_LIST_OT_Render_GenMotionFromTextPrompt(bpy.types.Operator): 
    """Move an item in the list.""" 
    bl_idname = "keemap_t2m.gen_motion_from_text_prompt" 
    ...
    def execute(self, context): 
        if context.scene.keemap_optm_traj_setting.text_prompt == "":
            self.report({'ERROR'}, "Must specify text prompt")
            return {'CANCELLED'}
        
        text_prompt = context.scene.keemap_optm_traj_setting.text_prompt
        target_joint = int(context.scene.keemap_optm_traj_setting.target_joint)
        seed = context.scene.keemap_optm_traj_setting.seed
        target_joint_1 = int(context.scene.keemap_optm_traj_setting.target_joint_1)
        ...
        if context.scene.keemap_optm_traj_setting.trajectory_obj != "":
            traj_curve_obj = bpy.data.objects.get(context.scene.keemap_optm_traj_setting.trajectory_obj)
            temp_obj = traj_curve_obj.copy()
            temp_obj.name = "Temp curve"
            ...
            vertices_mapping = {}
            for e in temp_obj.data.edges:
                vertices_mapping[e.vertices[0]] = e.vertices[1]
            # print([[e.vertices[0], e.vertices[1]] for e in temp_obj.data.edges])
            # traj_vert_list = [[vert.co[0], -vert.co[1], vert.co[2]] for vert in vertices]
            traj_vert_list = []
            cur_vert_idx = 0
            traj_vert_list.append([vertices[cur_vert_idx].co[0], -vertices[cur_vert_idx].co[1], vertices[cur_vert_idx].co[2]])
            while cur_vert_idx is not None:
                cur_vert_idx = vertices_mapping.get(cur_vert_idx, None)
                if cur_vert_idx is not None:
                    traj_vert_list.append([vertices[cur_vert_idx].co[0], -vertices[cur_vert_idx].co[1], vertices[cur_vert_idx].co[2]])
        ...
        if context.scene.keemap_optm_traj_setting.trajectory_obj_1 != "":
            traj_curve_obj = bpy.data.objects.get(context.scene.keemap_optm_traj_setting.trajectory_obj_1)
            temp_obj = traj_curve_obj.copy()
            temp_obj.name = "Temp curve"
            ...
            traj_vert_list_1 = []
            cur_vert_idx = 0
            traj_vert_list_1.append([vertices[cur_vert_idx].co[0], -vertices[cur_vert_idx].co[1], vertices[cur_vert_idx].co[2]])
            while cur_vert_idx is not None:
                cur_vert_idx = vertices_mapping.get(cur_vert_idx, None)
                if cur_vert_idx is not None:
                    traj_vert_list_1.append([vertices[cur_vert_idx].co[0], -vertices[cur_vert_idx].co[1], vertices[cur_vert_idx].co[2]])
        ...
        import os
        import sys
        proj_dir = "/home/edge/code/rebuttal/PartT2M-GPT-bestvq_cross_att_fix_linear_cfg_att_bias_fix_learnable_multiply_10.0_global_local_inplace_combine_att_bias_ignore_cfg_h8_same_decoder_no_output_linear_adamw_babel_novel_m_r/"
...
def register():
    bpy.utils.register_class(KeeMapOptmTrajSetting)
    bpy.types.Scene.keemap_optm_traj_setting = bpy.props.PointerProperty(type = KeeMapOptmTrajSetting) 
    bpy.utils.register_class(KEEMAP_LIST_OT_Render_GenMotionFromTextPrompt)

def unregister():
    bpy.utils.unregister_class(KEEMAP_LIST_OT_Render_GenMotionFromTextPrompt)
    bpy.utils.unregister_class(KeeMapOptmTrajSetting)    
    del bpy.types.Scene.keemap_optm_traj_setting 
